CREATE OR REPLACE PROCEDURE "polygon_parts".insert_part_mosaic_base(r "polygon_parts".insert_part_record)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    INSERT INTO "polygon_parts".orthophoto_mosaic_base_orthophoto_best_parts("record_id", "product_id", "product_type", "id", "name", "updated_in_version", "imaging_time_begin_utc", "imaging_time_end_utc", "resolution_degree", "resolution_meter", "source_resolution_meter", "horizontal_accuracy_ce_90", sensors, countries, cities, description, "geometry")
    VALUES(r.*);
END;
$$;

-- Usage example: CALL "polygon_parts".insert_part_mosaic_base(('795813b2-5c1d-466e-8f19-11c30d395fcd','WORLD_BASE', 'OrthophotoBest', '123', 'name', '5', '2022-08-22 02:08:10', '2022-08-22 02:08:10', 0.0001, 0.3, 0.3, 2.5, 'sensors', NULL, cities, 'description', 'SRID=4326;POLYGON((-20 51,10 51,10 56,-20 56,-20 51))')::"polygon_parts".insert_part_record);


-- PROCEDURE: polygon_parts.update_polygon_parts_mosaic_base()

-- DROP PROCEDURE IF EXISTS "polygon_parts".update_polygon_parts_mosaic_base();

CREATE OR REPLACE PROCEDURE "polygon_parts".update_polygon_parts_mosaic_base(
	)
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	...
	with unprocessed as (
		select "part_id", "record_id", "geometry" from "polygon_parts".orthophoto_mosaic_base_orthophoto_best_parts where not "is_processed_part" order by "part_id"
	)
	select 
		t1."internal_id",
		t1."part_id",
	...
	from (
		...
		from "polygon_parts".orthophoto_mosaic_base_orthophoto_best_polygon_parts pp
		join unprocessed
	...
	delete from "polygon_parts".orthophoto_mosaic_base_orthophoto_best_polygon_parts as pp
	using tbl
	where pp."internal_id" = tbl."internal_id";

	with unprocessed as (
		select * from "polygon_parts".orthophoto_mosaic_base_orthophoto_best_parts where not "is_processed_part" order by "part_id"
	), inserts as (
		select 
			"part_id",
			diff
	...
	insert into "polygon_parts".orthophoto_mosaic_base_orthophoto_best_polygon_parts as pp ("part_id", "record_id", "product_id", "product_type", "id", "name", "updated_in_version", "ingestion_date_utc", "imaging_time_begin_utc", "imaging_time_end_utc", "resolution_degree", "resolution_meter", "source_resolution_meter", "horizontal_accuracy_ce_90", sensors, countries, cities, description, "geometry")
	select 
		"part_id",
		"record_id",
		"product_id",
	...
	from (
		...
		from inserts
		left join "polygon_parts".orthophoto_mosaic_base_orthophoto_best_parts
		using ("part_id")
		union all
	...
	update "polygon_parts".orthophoto_mosaic_base_orthophoto_best_parts
	set "is_processed_part" = true
	where "is_processed_part" = false;
END;
$BODY$;
ALTER PROCEDURE "polygon_parts".update_polygon_parts_mosaic_base()
    OWNER TO postgres;
