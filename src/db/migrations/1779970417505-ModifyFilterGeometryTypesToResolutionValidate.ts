...
export class ModifyFilterGeometryTypesToResolutionValidate1779970417505 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        CREATE OR REPLACE FUNCTION polygon_parts.validate_resolutions(
            qualified_identifier_valid TEXT,   -- e.g. 'polygon_parts.valid'
            qualified_identifier_polygon_parts TEXT,   -- e.g. 'polygon_parts.parts'
            min_area_square_deg NUMERIC DEFAULT 1e-12  -- minimum intersection area threshold in square degrees
        )
        ...
        BEGIN
            ...
            sql := format($q$
                ...
                AND  ST_GeometryType(ST_Intersection(p.footprint, v.footprint)) IN ('ST_Polygon', 'ST_MultiPolygon')
                AND  ST_Area(ST_Intersection(p.footprint, v.footprint)) >= %L::numeric
                GROUP BY v.id, v.resolution_degree
            $q$, schema_valid, table_valid, schema_parts, table_parts, min_area_square_deg);
